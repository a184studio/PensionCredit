...
module.exports = function (req, res, next) {
  ...
  let passwords = [ process.env.PASSWORD ]

  if (process.env.EXTRA_PASSWORDS) {
    passwords = [ ...passwords, ...process.env.EXTRA_PASSWORDS.split(',') ];
  }
  ...
  if (env === 'production' && useAuth === 'true') {
    ...
    if (!user || user.name !== username || !passwords.includes(user.pass)) {
